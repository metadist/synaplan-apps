...
1. **TestFlight**: the job uploads the binary and then waits for Apple to finish processing, because
   that wait is what releases the build to the internal testers. Processing usually takes minutes
   but has taken over an hour; the job gives up after 45. Install the build, test on a real device.
   Apple reports a rejected binary by email and in App Store Connect.
   - A build that is selectable for submission but absent from TestFlight is processed and simply
     not distributed — the usual cause is a job that ended before the processing did. Add the build
     to the internal group by hand; a new build would change nothing.
