...
| FR | `frontend/src/views/OnboardingView.vue` | Native-only first-run onboarding flow (welcome → server → plans), step transitions + progress dots, `prefers-reduced-motion`-safe |
| FR | `frontend/src/components/onboarding/OnboardingWelcomeStep.vue`, `OnboardingServerStep.vue`, `OnboardingPlansStep.vue` | The three onboarding steps. Server step reuses the `nativeServer.ts` seam (probe/persist/reload stay app-owned); plans step reads the public `GET /api/v1/subscription/plans` and routes purchases through register → `/subscription` (native IAP path, never Stripe web checkout) |
