...
| FR | `frontend/src/views/OnboardingView.vue` | Native-only first-run onboarding flow (welcome → plans), page transitions + progress dots, `prefers-reduced-motion`-safe |
| FR | `frontend/src/components/onboarding/OnboardingWelcomeStep.vue`, `OnboardingServerModal.vue`, `OnboardingInfoModal.vue`, `OnboardingPlansStep.vue` | Page 1 (welcome) has a focused "get started" CTA plus quiet pills opening modals: the own-server modal (reuses the `nativeServer.ts` seam — probe/persist/reload stay app-owned) and RAG / chat-widget info modals. Page 2 (plans) reads the public `GET /api/v1/subscription/plans` and routes purchases through register → `/subscription` (native IAP path, never Stripe web checkout) |
| FR | `frontend/src/composables/useOnboarding.ts` | Completion flag (`localStorage['synaplan.onboardingCompleted']`), first-run gate (`shouldShowOnboarding`: native + signed-out + no guest session + not completed), resume-step persistence across the server-switch WebView reload |
...
| FR | `frontend/src/router/index.ts` | Public `/onboarding` route (lazy) + two guard branches: entry redirect into the first-run flow, and native-only/signed-out gating of the route itself | `shouldShowOnboarding()` is hard-false on web (`isNativeApp()`), for signed-in users, after completion/skip, and for existing guest sessions; direct web navigations to `/onboarding` bounce to chat/home — web behavior byte-identical |
