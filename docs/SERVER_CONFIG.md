...
| First-run onboarding | `synaplan/frontend/src/components/onboarding/OnboardingServerModal.vue` | Own-server modal opened from a quiet "Own server" pill on page 1 of the two-page native first-run flow (`/onboarding`): URL entry replacing the default server. Probes + persists via the same `window.SynaplanServer.save()` seam; the resulting WebView reload resumes the flow at page 1 (sessionStorage resume step) |
